...
The admin config stores `primaryAiProviderId`. The selected provider is tried first, followed by other enabled providers in configuration order. AI Provider API keys use the same env-var mechanism as data sources: config JSON stores only `apiKeyEnvName`, server-side reads the real value from `.env`, and admin API responses always redact `apiKey`. The current default provider is NVIDIA NIM through its OpenAI-compatible endpoint with `deepseek-ai/deepseek-v4-pro` and `NVIDIA_API_KEY`; NVIDIA requests are throttled to `NVIDIA_API_RPM=40` by default. DeepSeek and Xiaomi MiMo remain available as fallbacks.
